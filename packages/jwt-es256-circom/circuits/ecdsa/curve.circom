...
template PointOnLine(n, k, p) {
    ...
    var LOGK = bn_log_ceil(k);
    var LOGK2 = bn_log_ceil(3*k*k);
    assert(3*n + LOGK2 < 251);

    // AKA check point on line 
    component left = bn_BigMultShortLong(n, k, 2*n + LOGK + 1); // 2k-1 registers abs val < 2k*2^{2n}
    for(var i = 0; i < k; i++){
        left.a[i] <== in[0][1][i] + in[2][1][i];
        left.b[i] <== in[1][0][i] - in[0][0][i]; 
    }

    component right = bn_BigMultShortLong(n, k, 2*n + LOGK); // 2k-1 registers abs val < k*2^{2n}
    for(var i = 0; i < k; i++){
    ...
    component diff_red; 
    diff_red = bn_PrimeReduce(n, k, k-1, p, 3*n + LOGK2);
...
template PointOnCurve(n, k, a, b, p){
    ...
    var LOGK = bn_log_ceil(k);
    var LOGK2 = bn_log_ceil( (2*k-1)*(k*k+1) );
    assert(4*n + LOGK2 < 251);

    // compute x^3, y^2 
    component x_sq = bn_BigMultShortLong(n, k, 2*n + LOGK); // 2k-1 registers in [0, k*2^{2n}) 
    component y_sq = bn_BigMultShortLong(n, k, 2*n + LOGK); // 2k-1 registers in [0, k*2^{2n}) 
    for(var i=0; i<k; i++){
        x_sq.a[i] <== in[0][i];
        x_sq.b[i] <== in[0][i];
    ...
    component x_cu = bn_BigMultShortLongUnequal(n, 2*k-1, k, 3*n + 2*LOGK); // 3k-2 registers in [0, k^2 * 2^{3n}) 
    for(var i=0; i<2*k-1; i++)
        x_cu.a[i] <== x_sq.out[i];
    for(var i=0; i<k; i++)
        x_cu.b[i] <== in[0][i];

    component ax = bn_BigMultShortLong(n, k, 2*n + LOGK); // 2k-1 registers in [0, k*2^{2n})
    for (var i=0; i<k; i++) {
        ax.a[i] <== a[i];
        ax.b[i] <== in[0][i];
    }
    ...
    component cu_red = bn_PrimeReduce(n, k, 2*k-2, p, 4*n + 3*LOGK + 1);
    ...
    component y_sq_red = bn_PrimeReduce(n, k, k-1, p, 3*n + 2*LOGK + 1);
    for(var i=0; i<2*k-1; i++)
...
template PointOnTangent(n, k, a, p){
    ...
    var LOGK = bn_log_ceil(k);
    var LOGK3 = bn_log_ceil((3*k)*(2*k-1) + 1);
    assert(4*n + LOGK3 < 251);
    component x_sq = bn_BigMultShortLong(n, k, 2*n + LOGK); // 2k-1 registers < k*2^{2n}) 
    for(var i=0; i<k; i++){
        x_sq.a[i] <== in[0][0][i];
        x_sq.b[i] <== in[0][0][i];
    }
    component right = bn_BigMultShortLongUnequal(n, 2*k-1, k, 3*n + 2*LOGK + 3); // 3k-2 registers < (3*k+1)*k*2^{3n} 
    for(var i=0; i<2*k-1; i++){
        if (i < k) {
    ...
    component left = bn_BigMultShortLong(n, k, 2*n + 2 + LOGK); // 2k-1 registers in [0, 4k * 2^{2n})
    for(var i=0; i<k; i++){
        left.a[i] <== 2*in[0][1][i];
    ...
    component diff_red = bn_PrimeReduce(n, k, 2*k-2, p, 4*n + LOGK3);
    for(var i=0; i<3*k-2; i++){
        if(i < 2*k-1) 
...
template EllipticCurveAddUnequal(n, k, p) { 
    ...
    var LOGK = bn_log_ceil(k);
    var LOGK3 = bn_log_ceil( (3*k*k)*(2*k-1) + 1 ); 
    assert(4*n + LOGK3 < 251);

    // precompute lambda and x_3 and then y_3
    var dy[50] = bn_long_sub_mod(n, k, b[1], a[1], p);
    var dx[50] = bn_long_sub_mod(n, k, b[0], a[0], p); 
    var dx_inv[50] = bn_mod_inv(n, k, dx, p);
    var lambda[50] = bn_prod_mod(n, k, dy, dx_inv, p);
    var lambda_sq[50] = bn_prod_mod(n, k, lambda, lambda, p);
    // out[0] = x_3 = lamb^2 - a[0] - b[0] % p
    // out[1] = y_3 = lamb * (a[0] - x_3) - a[1] % p
    var x3[50] = bn_long_sub_mod(n, k, bn_long_sub_mod(n, k, lambda_sq, a[0], p), b[0], p);
    var y3[50] = bn_long_sub_mod(n, k, bn_prod_mod(n, k, lambda, bn_long_sub_mod(n, k, a[0], x3, p), p), a[1], p);
    ...
    component dx_sq = bn_BigMultShortLong(n, k, 2*n+LOGK+2); // 2k-1 registers abs val < k*2^{2n} 
    component dy_sq = bn_BigMultShortLong(n, k, 2*n+LOGK+2); // 2k-1 registers < k*2^{2n}
    for(var i = 0; i < k; i++){
    ...
    component cubic = bn_BigMultShortLongUnequal(n, k, 2*k-1, 3*n+4+2*LOGK); // 3k-2 registers < 3 * k^2 * 2^{3n} ) 
    for(var i=0; i<k; i++)
    ...
    component cubic_red = bn_PrimeReduce(n, k, 2*k-2, p, 4*n + LOGK3);
    for(var i=0; i<2*k-1; i++)
...
template EllipticCurveDouble(n, k, a, b, p) {
    ...
    // precompute lambda 
    var lamb_num[50] = bn_long_add_mod(n, k, a, bn_prod_mod(n, k, long_3, bn_prod_mod(n, k, in[0], in[0], p), p), p);
    var lamb_denom[50] = bn_long_add_mod(n, k, in[1], in[1], p);
    var lamb[50] = bn_prod_mod(n, k, lamb_num, bn_mod_inv(n, k, lamb_denom, p), p);

    // precompute x_3, y_3
    var x3[50] = bn_long_sub_mod(n, k, bn_prod_mod(n, k, lamb, lamb, p), bn_long_add_mod(n, k, in[0], in[0], p), p);
    var y3[50] = bn_long_sub_mod(n, k, bn_prod_mod(n, k, lamb, bn_long_sub_mod(n, k, in[0], x3, p), p), in[1], p);

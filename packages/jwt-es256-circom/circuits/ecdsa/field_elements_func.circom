...
function find_Fp_inverse(n, k, num, p) {
    var amodp[2][50] = bn_long_div2(n, k, 0, num, p); 
    var a[50];
    var b[50]; 
    var x[50];
    var y[50];
    ...
    for(var l=0; l<1000; l++){
        ...
        var r[2][50] = bn_long_div2(n, ka, k - ka, b, a); 
        var q[50]; 
        ...
        var newu[50] = bn_long_sub_mod(n, k, x, bn_prod_mod(n, k, u, q, p), p); 
        var newv[50] = bn_long_sub_mod(n, k, y, bn_prod_mod(n, k, v, q, p), p); 
...
function get_signed_Fp_carry_witness(n, k, m, a, p){
    var out[2][50];
    var a_short[51] = bn_signed_long_to_short(n, k, a); 
    ...
    */
    ...
    if(a_short[50] == 0){
        out = bn_long_div2(n, k, m, a_short, p);    
    }else{
        var a_pos[50];
        for(var i=0; i<k+m; i++) 
            a_pos[i] = -a_short[i];

        var X[2][50] = bn_long_div2(n, k, m, a_pos, p);
        // what if X[1] is 0? 
        var Y_is_zero = 1;
        for(var i=0; i<k; i++){
            if(X[1][i] != 0)
...
function find_Fp2_product(n, k, a, b, p){
    ...
    for(var i=0; i<2; i++)for(var j=0; j<2; j++){
        ab[i][j] = bn_prod_mod(n,k,a[i],b[j],p);
    }
    out[0] = bn_long_sub_mod(n,k,ab[0][0],ab[1][1],p); 
    out[1] = bn_long_add_mod(n,k,ab[0][1],ab[1][0],p);
...
function find_Fp2_sum(n, k, a, b, p){
    var out[2][50];
    out[0] = bn_long_add_mod(n,k,a[0],b[0],p); 
    out[1] = bn_long_add_mod(n,k,a[1],b[1],p);
    return out;
}
...
function find_Fp2_diff(n, k, a, b, p){
    var out[2][50];
    out[0] = bn_long_sub_mod(n,k,a[0],b[0],p); 
    out[1] = bn_long_sub_mod(n,k,a[1],b[1],p);
    return out;
}
...
function find_Fp2_inverse(n, k, a, p) {
    var sq0[50] = bn_prod(n, k, a[0], a[0]);
    var sq1[50] = bn_prod(n, k, a[1], a[1]);
    var sq_sum[50] = bn_long_add(n, 2*k, sq0, sq1);
    var sq_sum_div[2][50] = bn_long_div2(n, k, k+1, sq_sum, p);
    // lambda = 1/(sq_sum)%p
    var lambda[50] = bn_mod_inv(n, k, sq_sum_div[1], p);
    var out0[50] = bn_prod(n, k, lambda, a[0]);
    var out0_div[2][50] = bn_long_div(n, k, out0, p);
    var out[2][50];
    out[0] = out0_div[1];
    
    var out1_pre[50] = bn_long_sub(n, k, p, a[1]);
    var out1[50] = bn_prod(n, k, lambda, out1_pre);
    var out1_div[2][50] = bn_long_div(n, k, out1, p);
    out[1] = out1_div[1];
    return out;
}

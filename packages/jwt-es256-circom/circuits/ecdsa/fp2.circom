...
template SignedFp2MultiplyNoCarryUnequal(n, ka, kb, m_out){
    ...
    for(var i=0; i<2; i++)for(var j=0; j<2; j++){
        ab[i][j] = bn_BigMultShortLongUnequal(n, ka, kb, m_out); // output has ka+kb-1 registers
        for(var l=0; l<ka; l++)
            ab[i][j].a[l] <== a[i][l];
        for(var l=0; l<kb; l++)
            ab[i][j].b[l] <== b[j][l];
...
template Fp2Compress(n, k, m, p, m_out){
    ...
    component c[2];
    for(var i=0; i<2; i++){
        c[i] = bn_PrimeReduce(n, k, m, p, m_out);
        for(var j=0; j<k+m; j++)
            c[i].in[j] <== in[i][j]; 
    }
    for(var i=0; i<2; i++)for(var j=0; j<k; j++)
...
template Fp2MultiplyThree(n, k, p){
    ...
    var LOGK3 = bn_log_ceil(4*k*k*(2*k-1));
...
template SignedFp2Divide(n, k, overflowa, overflowb, p){
    ...
    component mult = SignedFp2MultiplyNoCarryCompress(n, k, p, bn_max(n, overflowb), 2*n + overflowb + LOGK2); 
    for(var eps=0; eps<2; eps++)for(var i=0; i<k; i++){
        mult.a[eps][i] <== out[eps][i]; 
        mult.b[eps][i] <== b[eps][i]; 
    }
    
    var m = bn_max( mb + k, ma );
    ...
    var overflow = bn_max(2*n + overflowb + LOGK2, overflowa);
...
template Fp2IsEqual(n, k, p){
    ...
    var total = 2*k;
    for(var j=0; j<2; j++){
        lta[j] = bn_BigLessThan(n, k);
        ltb[j] = bn_BigLessThan(n, k);
        for (var i = 0; i < k; i ++) {
            lta[j].a[i] <== a[j][i]; 
            lta[j].b[i] <== p[i]; 

...
template bn_BigAdd(n, k) {
    assert(n <= 252);
    signal input a[k];
    signal input b[k];
    signal output out[k + 1];

    component unit0 = bn_ModSum(n);
    unit0.a <== a[0];
    unit0.b <== b[0];
    out[0] <== unit0.sum;
...
*/
template bn_BigMultShortLong(n, k, m_out) {
   assert(n <= 126);
   signal input a[k];
   signal input b[k];
...
*/
template bn_BigMultShortLongUnequal(n, ka, kb, m_out) {
    assert(n <= 126);
    signal input a[ka];
    signal input b[kb];
...
template bn_LongToShortNoEndCarry(n, k) {
    assert(n <= 126);
    signal input in[k];
    signal output out[k+1];
    ...
    var split[k][3];
    for (var i = 0; i < k; i++) {
        split[i] = bn_SplitThreeFn(in[i], n, n, n);
    }
...
template bn_BigMult(n, k) {
    signal input a[k];
    signal input b[k];
    signal output out[2 * k];

    var LOGK = bn_log_ceil(k);
    component mult = bn_BigMultShortLong(n, k, 2*n + LOGK);
    for (var i = 0; i < k; i++) {
        mult.a[i] <== a[i];
        mult.b[i] <== b[i];
    }
    ...
    component longshort = bn_LongToShortNoEndCarry(n, 2 * k - 1);
    for (var i = 0; i < 2 * k - 1; i++) {
...
template bn_BigMod(n, k) {
    ...
    var longdiv[2][50] = bn_long_div(n, k, a, b);
    for (var i = 0; i < k; i++) {
        div[i] <-- longdiv[0][i];
        mod[i] <-- longdiv[1][i];
    }
    ...
    component mul = bn_BigMult(n, k + 1);
    for (var i = 0; i < k; i++) {
        mul.a[i] <== div[i];
        mul.b[i] <== b[i];
    }
    ...
    component add = bn_BigAdd(n, 2 * k + 2);
    for (var i = 0; i < 2 * k; i++) {
        add.a[i] <== mul.out[i];
        if (i < k) {
            add.b[i] <== mod[i];
    ...
    component lt = bn_BigLessThan(n, k);
    for (var i = 0; i < k; i++) {
        lt.a[i] <== mod[i];
        lt.b[i] <== b[i];
    }
...
template bn_BigMod2(n, k, m) {
    ...
    var longdiv[2][50] = bn_long_div2(n, k, m-k, a, b);
    for (var i = 0; i < k; i++) {
        mod[i] <-- longdiv[1][i];
    }
    for (var i = 0; i <= m-k; i++) {
    ...
    component mul = bn_BigMult(n, m-k + 1);
...
template bn_BigAddModP(n, k){
    ...
    component add = bn_BigAdd(n,k);
    for (var i = 0; i < k; i++) {
        add.a[i] <== a[i];
        add.b[i] <== b[i];
    }
    component lt = bn_BigLessThan(n, k+1);
    for (var i = 0; i < k; i++) {
        lt.a[i] <== add.out[i];
        lt.b[i] <== p[i];
    }
...
template bn_BigSubModP(n, k){
    assert(n <= 252);
    signal input a[k];
    signal input b[k];
    signal input p[k];
    signal output out[k];
    component sub = bn_BigSub(n, k);
    for (var i = 0; i < k; i++){
        sub.a[i] <== a[i];
        sub.b[i] <== b[i];
    }
...
template bn_BigMultModP(n, k) {
    ...
    component big_mult = bn_BigMult(n, k);
    for (var i = 0; i < k; i++) {
        big_mult.a[i] <== a[i];
        big_mult.b[i] <== b[i];
    }
    component big_mod = bn_BigMod(n, k);
    for (var i = 0; i < 2 * k; i++) {
...
template bn_BigModInv(n, k) {
    ...
    // length k
    var inv[50] = bn_mod_inv(n, k, in, p);
    for (var i = 0; i < k; i++) {
        out[i] <-- inv[i];
    }
    ...
    component mult = bn_BigMult(n, k);
    for (var i = 0; i < k; i++) {
        mult.a[i] <== in[i];
        mult.b[i] <== out[i];
    }
    component mod = bn_BigMod(n, k);
    for (var i = 0; i < 2 * k; i++) {
        mod.a[i] <== mult.out[i];
    }
    for (var i = 0; i < k; i++) {
...
*/
template bn_CheckCarryToZero(n, m, k) {
    assert(k >= 2);
...
m_out is the expected max number of bits in the output registers
*/
template bn_PrimeReduce(n, k, m, p, m_out){
    signal input in[m+k]; 
    signal output out[k];
    ...
    e[0] = n;
    var pow2n[50] = bn_mod_exp(n, k, two, p, e); 
    e[0] = k;
    assert(k < (1<<n) );
    var pow2nk[50] = bn_mod_exp(n, k, pow2n, p, e);
    ...
    for(var i=0; i<m; i++){
        ...
        if(i==0){
            ...
            r[i] = bn_prod_mod(n, k, r[i-1], pow2n, p);  
        }
    } 
    var out_sum[k]; 
    for(var i=0; i<k; i++)
...
*/
template bn_BigMultShortLong2D(n, k, l) {
    signal input a[l][k];
    signal input b[l][k];
...
*/
template bn_BigMultShortLong2DUnequal(n, ka, kb, la, lb) {

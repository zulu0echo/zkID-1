...
template ECDSAPrivToPub(n, k) {
    ...
    var num_strides = ecdsa_div_ceil(n * k, stride);
...
template ECDSAVerifyNoPubkeyCheck(n, k) {
    ...
    var sinv_comp[50] = bn_mod_inv(n, k, s, order);
    signal sinv[k];
    ...
    component sinv_check = bn_BigMultModP(n, k);
    for (var idx = 0; idx < k; idx++) {
    ...
    component g_coeff = bn_BigMultModP(n, k);
    for (var idx = 0; idx < k; idx++) {
    ...
    component pubkey_coeff = bn_BigMultModP(n, k);
    for (var idx = 0; idx < k; idx++) {

...
function bn_SplitFn(in, n, m) {
    return [in % (1 << n), (in \ (1 << n)) % (1 << m)];
}

function bn_SplitThreeFn(in, n, m, k) {
    return [in % (1 << n), (in \ (1 << n)) % (1 << m), (in \ (1 << n + m)) % (1 << k)];
}

// 1 if true, 0 if false
function bn_long_gt(n, k, a, b) {
    for (var i = k - 1; i >= 0; i--) {
        if (a[i] > b[i]) {
            return 1;
        }
...
function bn_long_add(n, k, a, b){
    var carry = 0;
    var sum[50];
    for(var i=0; i<k; i++){
        var sumAndCarry[2] = bn_SplitFn(a[i] + b[i] + carry, n, n);
        sum[i] = sumAndCarry[0];
        carry = sumAndCarry[1];
    }
    sum[k] = carry;
...
// d has k registers
// output has k+1 registers
function bn_long_add4(n, k, a, b, c, d){
    var carry = 0;
    var sum[50];
    for(var i=0; i < k; i++){
        var sumAndCarry[2] = bn_SplitFn(a[i] + b[i] + c[i] + d[i] + carry, n, n);
        sum[i] = sumAndCarry[0];
        carry = sumAndCarry[1];
    }
    sum[k] = carry;
...
function bn_long_add_unequal(n, k1, k2, a, b){
    var carry = 0;
    var sum[50];
    for(var i=0; i<k1; i++){
        if (i < k2) {
            ...
            var sumAndCarry[2] = bn_SplitFn(a[i] + carry, n, n);
            sum[i] = sumAndCarry[0];
            carry = sumAndCarry[1];
        }
    }
...
// a is a n-bit scalar
// b has k registers
function bn_long_scalar_mult(n, k, a, b) {
    var out[50];
    for (var i = 0; i < 50; i++) {
        out[i] = 0;
    }
...
function bn_long_div2(n, k, m, a, b){
    var out[2][50];
    // assume k+m < 50
    var remainder[50];
    for (var i = 0; i < m + k; i++) {
    ...
    for (var i = m; i >= 0; i--) {
        ...
        out[0][i] = bn_short_div(n, k, dividend, b);
        var mult_shift[50] = bn_long_scalar_mult(n, k, out[0][i], b);
        var subtrahend[50];
        for (var j = 0; j < m + k; j++) {
            subtrahend[j] = 0;
        }
        ...
        remainder = bn_long_sub(n, m + k, remainder, subtrahend);
    }
    for (var i = 0; i < k; i++) {
        out[1][i] = remainder[i];
    }
...
function bn_long_div(n, k, a, b) {
    return bn_long_div2(n, k, k, a, b);
}
...
// 0 <= a < (2**n) * b
function bn_short_div_norm(n, k, a, b) {
   var qhat = (a[k] * (1 << n) + a[k - 1]) \ b[k - 1];
   if (qhat > (1 << n) - 1) {
      qhat = (1 << n) - 1;
   }

   var mult[50] = bn_long_scalar_mult(n, k, qhat, b);
   if (bn_long_gt(n, k + 1, mult, a) == 1) {
      mult = bn_long_sub(n, k + 1, mult, b);
      if (bn_long_gt(n, k + 1, mult, a) == 1) {
         return qhat - 2;
      } else {
         return qhat - 1;
...
// 0 <= a < b * 2^n
function bn_short_div(n, k, a, b) {
    var scale = (1 << n) \ (1 + b[k - 1]);
    // k + 2 registers now
    var norm_a[50] = bn_long_scalar_mult(n, k + 1, scale, a);
    // k + 1 registers now
    var norm_b[50] = bn_long_scalar_mult(n, k, scale, b);
    
    var ret;
    if (norm_b[k] != 0) {
	ret = bn_short_div_norm(n, k + 1, norm_a, norm_b);
    } else {
	ret = bn_short_div_norm(n, k, norm_a, norm_b);
    }
    return ret;
}
...
function bn_signed_long_to_short(n, k, a){
    var out[51];
    var MAXL = 50;
    var temp[51];
...
function bn_prod(n, k, a, b) {
    ...
    for (var i = 0; i < 2 * k - 1; i++) {
        split[i] = bn_SplitThreeFn(prod_val[i], n, n, n);
    }
    ...
    out[0] = split[0][0];
    if (2 * k - 1 > 1) {
        var sumAndCarry[2] = bn_SplitFn(split[0][1] + split[1][0], n, n);
        out[1] = sumAndCarry[0];
        carry[1] = sumAndCarry[1];
    }
    if (2 * k - 1 > 2) {
        for (var i = 2; i < 2 * k - 1; i++) {
            var sumAndCarry[2] = bn_SplitFn(split[i][0] + split[i-1][1] + split[i-2][2] + carry[i-1], n, n);
            out[i] = sumAndCarry[0];
...
function bn_prod2D(n, k, l, a, b) {
    ...
    for (var j = 0; j < 2 * l - 1; j ++) {
        for (var i = 0; i < 2 * k - 1; i++) {
            split[j][i] = bn_SplitThreeFn(prod_val[j][i], n, n, n);
        }
    }
    ...
    for ( var j = 0; j < 2 * l - 1; j ++) {
        ...
        out[j][0] = split[j][0][0];
        if (2 * k - 1 > 1) {
            sumAndCarry[j] = bn_SplitFn(split[j][0][1] + split[j][1][0], n, n);
            out[j][1] = sumAndCarry[j][0];
            carry[j][1] = sumAndCarry[j][1];
        }
        if (2 * k - 1 > 2) {
            for (var i = 2; i < 2 * k - 1; i++) {
                sumAndCarry[j] = bn_SplitFn(split[j][i][0] + split[j][i-1][1] + split[j][i-2][2] + carry[j][i-1], n, n);
...
function bn_long_add_mod(n, k, a, b, p) {
    var sum[50] = bn_long_add(n,k,a,b); 
    var temp[2][50] = bn_long_div2(n,k,1,sum,p);
    return temp[1];
}

function bn_long_sub_mod(n, k, a, b, p) {
    if(bn_long_gt(n, k, b, a) == 1){
        return bn_long_add(n, k, a, bn_long_sub(n,k,p,b));
    }else{
        return bn_long_sub(n, k, a, b);
    }
}

function bn_prod_mod(n, k, a, b, p) {
    var prod[50] = bn_prod(n,k,a,b);
    var temp[2][50] = bn_long_div(n,k,prod,p);
    return temp[1];
}
...
function bn_mod_exp(n, k, a, p, e) {
    ...
    for (var i = bitlength-1; i >= 0; i--) {
        ...
        if (eBits[i] == 1) {
            ...
            temp = bn_prod(n, k, out, a);
            var temp2[2][50];
            temp2 = bn_long_div(n, k, temp, p);
            out = temp2[1];
        }
...
function bn_mod_inv(n, k, a, p) {
    ...
    var pMinusTwo[50];
    pMinusTwo = bn_long_sub(n, k, pCopy, two); // length k
    var out[50];
    out = bn_mod_exp(n, k, a, pCopy, pMinusTwo);
    return out;
}

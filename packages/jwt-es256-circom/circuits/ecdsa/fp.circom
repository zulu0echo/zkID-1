...
template FpNegate(n, k, p){
    ...
    component neg = bn_BigSub(n, k); 
    component is_zero = bn_BigIsZero(k);
...
template FpMultiply(n, k, p) {
    ...
    component nocarry = bn_BigMultShortLong(n, k, 2*n + LOGK);
    for (var i = 0; i < k; i++) {
        nocarry.a[i] <== a[i];
        nocarry.b[i] <== b[i];
    }
    component red = bn_PrimeReduce(n, k, k-1, p, 3*n + 2*LOGK);
...
template CheckCarryModP(n, k, m, overflow, p){
    ...
    pX = bn_BigMultShortLongUnequal(n, k, m, overflow); // p has k registers, X has m registers, so output really has k+m-1 registers 
    ...
    carry_check = bn_CheckCarryToZero(n, overflow+1, k+m-1 ); 
    for(var i=0; i<k; i++){
...
template FpIsZero(n, k, p){
    ...
    // check that in < p 
    component lt = bn_BigLessThan(n, k);
    component isZero = bn_BigIsZero(k);
    for(var i = 0; i < k; i++) {
        lt.a[i] <== in[i];
        lt.b[i] <== p[i];
